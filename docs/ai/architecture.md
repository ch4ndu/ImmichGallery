...
- Timeline Mosaic assignments are precomputed after successful server sync for changed buckets, persisted in Room, and loaded by `TimelineViewModel` as display input. Persisted assignment rows are keyed by bucket or section, group mode, fixed Timeline Mosaic column count, asset fingerprint, and enabled Mosaic families. Width-dependent Timeline Mosaic display-cache rows store portable real/fallback band records for the active config and current geometry width; keep them separate from width-independent assignments. Do not let assignments or display cache cross day-section boundaries. Runtime reads of persisted Mosaic rows should be scoped to materialized visible/nearby buckets and guarded by generation/config checks so stale cache reads cannot publish after the screen moves or the Mosaic config changes.
...
- Timeline Mosaic uses fixed columns while enabled: 4 on normal widths and 5 on large-width layouts. Pinch/desktop zoom should not change Timeline column count while Mosaic is enabled, and the Timeline grouping control should be disabled while Mosaic is enabled.
