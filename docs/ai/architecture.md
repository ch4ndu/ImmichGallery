...
- Timeline Mosaic placeholder geometry protects scroll state. Persist width-keyed section geometry and aggregate bucket geometry when assignments are precomputed and a measured Timeline width is available. Timeline is edge-to-edge, so geometry uses the full Timeline bounds under system/top/bottom bars; bar padding affects interaction and scrollbars, not Mosaic geometry. Cold sync fuses asset write, assignment precompute, section geometry, and aggregate bucket geometry per bucket; exact aggregate geometry lets unloaded buckets render accurate placeholders without hydrating all assets into memory. In day mode, aggregate bucket geometry must include day headers and inter-item spacing. Aggregate geometry runtime keys must include group mode and the full geometry request identity, and stale aggregate reads must be generation/config guarded. Count-only placeholders are a last fallback for buckets with metadata but no exact geometry or materialized asset data. Superseded persisted Mosaic reads during rapid scroll are expected cancellation, not user-facing load failures.
