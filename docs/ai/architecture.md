...
- Mosaic layout sizing flows through `MosaicLayoutSpec`: Timeline, Album Detail, and Person Detail use the global `ViewConfig.mosaicColumnCount` while Mosaic is enabled. Mosaic bands, placeholders, and fallback rows use `availableWidth / columnCount` as the effective cell height.
...
- `MosaicWorkScheduler` in `ui/util/` is the shared priority gate for CPU-heavy Album Detail and Person Detail Mosaic assignment work. Foreground visible detail groups outrank foreground prefetch, and newly visible owner work may cancel same-owner offscreen prefetch so visible work does not wait behind a large running group. Detail screens should reprioritize work on visible-group changes without treating scroll as a new layout generation. Album/Person detail Mosaic rendering goes through the detail renderer seam: `CachedPhotoGridMosaicRenderer` reads persisted group display bands when `ViewConfig.cacheMosaicResults` is enabled and the owner snapshot is complete, while `RuntimePhotoGridMosaicRenderer` computes missing groups and writes each completed group back to cache asynchronously after publishing UI.
...
- Timeline content invalidation is per bucket. Cached launch syncs bucket metadata only after a Timeline cold-complete marker exists in `sync_metadata`; bucket metadata alone is not enough because failed cold syncs may leave partial metadata. Cached Room refs, exact bucket geometry, and materialized grid rows are separate states. Keep bucket metadata available for scroll height, but read Room assets and build rows only for visible/nearby buckets. Cold first sync is still blocking, but it must publish exact bucket placeholders without marking every bucket loaded, then set the cold-complete marker. A successful sync must not bump a global asset revision or repack unchanged buckets; only buckets whose ordered assets changed should invalidate derived rows and persisted Mosaic assignments. Removed buckets still clear cached refs immediately. Album and Person detail use the same ordered asset fingerprint idea for their single-screen asset revision.
- Timeline visible/target bucket loading is the highest-priority render path. Normal scroll, settled scroll, and scrollbar targets should materialize cached assets for the visible or explicitly targeted bucket window before offscreen work, then move those buckets to the front of any serial server-refresh queue. Scrollbar target callbacks are authoritative during fast-scroll, including mid-bucket positions where no month header is visible; drag-stop buckets outrank earlier drag targets and cached materialization must not wait behind server refresh. Do not require the user to cross a bucket boundary before a scrollbar-targeted bucket starts loading.
- Timeline Mosaic assignments are precomputed after successful server sync for changed buckets, persisted in Room, and loaded by `TimelineViewModel` as display input. `TimelineRepository` owns bucket metadata and asset refs; `TimelineMosaicCacheRepository` owns Timeline Mosaic assignments, display-cache rows, section geometry, aggregate bucket geometry, and Mosaic cache clear. Persisted assignment rows are keyed by bucket or section, group mode, configured Mosaic column count, asset fingerprint, and enabled Mosaic families. Width-dependent Timeline Mosaic display-cache rows store portable real/fallback band records for the active config and current geometry width; keep them separate from width-independent assignments. Do not let assignments or display cache cross day-section boundaries. Runtime reads of persisted Mosaic rows should be scoped to materialized visible/nearby buckets and guarded by generation/config checks so stale cache reads cannot publish after the screen moves or the Mosaic config changes. Timeline Mosaic cache read use cases are pure reads and must not backfill geometry or display-cache rows. `ViewConfig.cacheMosaicResults = false` bypasses Timeline display-band cache reads, but it must not disable Timeline assignment or geometry reads because those preserve fast-scroll placeholder accuracy.
- Missing or changed Timeline Mosaic rows may emit session-only progressive chunks while the full assignment pass is still computing. Chunks batch stable Mosaic bands, own contiguous source-asset ranges, remain in ViewModel runtime state only, and must be ignored when bucket revision, group mode, column count, families, or full-screen geometry identity changes. Progressive chunks should be mutex-buffered, deduplicated by source range, throttled before UI publication, and flushed immediately for visible/nearby or scrollbar-targeted buckets. Complete persisted rows still replace partial chunks when available; do not add chunk persistence or a memory cap without an explicit cache/schema decision.
- Timeline Mosaic placeholder geometry protects scroll state. Persist width-keyed section geometry, display-cache bands, and aggregate bucket geometry when assignments are precomputed and a measured Timeline width is available. Timeline is edge-to-edge, so geometry uses the full Timeline bounds under system/top/bottom bars; bar padding affects interaction and scrollbars, not Mosaic geometry. Cold sync fuses asset write, assignment precompute, display-cache generation, section geometry, and aggregate bucket geometry per bucket; exact aggregate geometry lets unloaded buckets render accurate placeholders without hydrating all assets into memory. In day mode, aggregate bucket geometry must include day headers and inter-item spacing. Aggregate geometry runtime keys must include group mode and the full geometry request identity, and stale aggregate reads must be generation/config guarded. Count-only placeholders are a last fallback for buckets with metadata but no exact geometry or materialized asset data. Superseded persisted Mosaic reads during rapid scroll are expected cancellation, not user-facing load failures.
- Mosaic column count is changed only through the Mosaic settings dialog. When `ViewConfig.cacheMosaicResults` is true, applying Mosaic changes must block in the dialog until the full applicable cache is prepared: every current Timeline metadata bucket for Timeline, every group for Album Detail, and the full loaded Person Detail owner after full person sync. Timeline blocking prepare runs asset sync first, retries failed buckets once, then runs explicit Mosaic cache preparation before reporting readiness; an empty Room asset read is not cache readiness when bucket metadata still reports assets. If preparation fails, do not persist or apply the draft config. The Timeline grouping control stays disabled while Mosaic is enabled.
...
- Album and Person detail screens publish placeholder display items immediately while foreground Mosaic groups wait for the scheduler or cache miss fallback. Visible groups should be submitted before offscreen prefetch groups, pending work should be reprioritized when visibility changes, and stale layout generations must not publish results. Runtime detail Mosaic should publish visible or near-visible completed groups immediately, defer offscreen group replacement while the list is actively scrolling, and batch deferred replacements after scroll settles. Cached placeholder height should be used when available so placeholder-to-Mosaic replacement does not shift the list unnecessarily. Album Detail may read/write owner cache whenever Mosaic cache is enabled; Person Detail may read/write owner cache only when the full asset set is known (`hasMore == false`) so partial pages never persist incomplete geometry.
...
- Timeline Mosaic fallback must render `MosaicBandItem` output, not justified `RowItem` output, while Mosaic is enabled. Unassigned, invalid, missing, and failed ranges use deterministic full-width fallback bands with fit-style thumbnails; visible blank space is acceptable. The assignment scanner advances by one asset when no valid template exists at a source index instead of using row packing as a skip heuristic. Album and Person detail Mosaic fallback may keep their existing justified-row policy unless deliberately changed in that screen family.
- Timeline bucket-load display updates must stay immediate for shared-element return transitions; debounce only zoom/config layout work. `TimelineState` carries a precomputed display index for scroll targeting, section-label lookup, visible-bucket detection, and first-visible-asset anchoring so hot UI paths do not rescan `displayItems`; Album and Person detail use the same display-index pattern for visible Mosaic group detection. Timeline lazy-list keys must include bucket and section identity for placeholders and Mosaic bands, because shared grid builder keys can be section-local. Sticky headers should read labels from the display index and retain the last non-null label during transient progressive updates. The Timeline detail overlay should collect a projected Timeline state containing only bucket/page data and keep using the shared `bucketAssetsCache` for transition continuity.
