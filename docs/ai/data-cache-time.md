...
- App database version bumps may rely on destructive migration while the project remains a pre-alpha cache-only client. Old Mosaic cache rows must not be reused after artifact shape changes; a full cold resync is acceptable. Version 15 resets interrupted or pre-completed fallback Mosaic display rows after the completed-only fallback policy change.
...
- Timeline buckets load lazily. Cached launch syncs Timeline bucket metadata only; cached Room refs only mark asset availability. The ViewModel should materialize cached asset rows into memory for visible/nearby buckets, keep cached rows visible during background refresh, and avoid render-facing loading/error mutations for unchanged cached refreshes. Unchanged already-loaded refreshes should not republish bucket state or enqueue Mosaic reads. Visible/nearby bucket asset refresh is serial and cache-preserving, while top-bar refresh performs the explicit full Timeline asset refresh.
