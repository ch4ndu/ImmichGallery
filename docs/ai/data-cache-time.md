...
- Timeline Mosaic cache rows are split by purpose: assignments are width-independent, while section geometry, aggregate bucket geometry, and mixed display-record cache rows are width/key/version dependent. `PrepareTimelineMosaicCacheAction` and `TimelineMosaicArtifactBuilder` derive Timeline Mosaic artifacts through `MosaicRenderEngine`; `TimelineMosaicCacheRepository` persists, reads, clears, and validates these rows only. `TimelineRepository` stays bucket/asset-sync focused but may expose a bucket snapshot read of ordered assets plus metadata count for precompute. `ViewConfig.cacheMosaicResults = false` disables sync-time Timeline Mosaic artifact preparation and disk reads for rendering; materialized Timeline buckets compute runtime Mosaic state for the requested config only from render-demand work after active scrolling settles instead of fetching old rows from disk.
- Album and Person detail Mosaic cache uses owner-scoped assignment, mixed display-record, section-geometry, and aggregate-geometry artifacts. Cache keys must include owner identity, group/section identity when section-scoped, Mosaic column count for every artifact type, normalized Mosaic families, ordered asset fingerprint, rounded geometry keys, and artifact version. Album Detail may cache whenever the owner snapshot is complete; Person Detail must only persist/read owner Mosaic cache after the full asset set is known (`hasMore == false`).
...
- Mosaic cache readiness requires matching assignments, section geometry, and aggregate geometry. Mixed display rows are additionally required only when `ViewConfig.cacheMosaicResults = true`, and display rows must independently cover the current ordered assets with no gaps, overlaps, duplicate ids, unknown ids, invalid real-tile dimensions, invalid fallback-row heights, invalid real-band visual order, or mismatched source slices. Fallback display rows must preserve source asset order exactly. Timeline and detail runtime rendering must not read or reuse disk Mosaic artifacts when cache results are disabled; they should compute Mosaic display for the requested config even if matching rows exist. Timeline active-scroll persisted cache reads are visible/target scoped and must not replace display rows until scroll settles. Detail runtime partial chunks, retryable fallbacks, and assignment checkpoints are memory-only state and are not cache readiness. Repositories must not compute Mosaic assignments, fallback rows, display projection, geometry, or artifact payloads directly; they only persist, read, clear, and validate artifacts produced through the domain engine/builder.
