...
- Detail still-photo viewing should keep one stable zoomable full-resolution image path visible outside active transition/drag. Drag-dismiss transforms belong on the fitted media container, using tap-anchored layout size/offset so the touched media point stays under the finger; do not rely on draw-only `graphicsLayer` for the exit source rect. Pinch zoom takes precedence over drag before vertical dismiss commits. Freeze committed dismiss transforms until overlay unmount, use a short full-image alpha handoff for still-photo open/dismiss transitions, keep the shared thumbnail synchronously visible during real transitions, and only reset transforms to identity for snap-back/cancel. Drag dismiss freezes the detail-side shared thumbnail at the released media bounds; back and shortcut dismisses freeze the current page but keep normal centered media bounds. Photo overlay transition participation and grid hiding are separate states: only the active transition asset may mount shared-element thumbnail machinery, non-participating cells stay on the plain thumbnail path, and overlay dismiss must synchronously reveal the grid-side source before clearing overlay state. Keep shared bounds tweening disabled during dismiss source prep while the grid source and detail node overlap; enable it for the actual overlay exit so drag exits fly from released bounds instead of recentering first. Off-screen overlay dismiss source-prep must only reveal the source row or Mosaic band: already-visible sources are not repositioned, sources above the viewport become first visible, sources below the viewport become last visible when shorter than the viewport, then the active thumbnail mounts and reports fresh measured bounds before overlay state clears when possible. Detail-side shared thumbnails should use the same bounded exact thumbnail request and cache keys as grid thumbnails to avoid empty destination frames. Ignore overlay page-current callbacks after dismiss begins so the grid source cannot be hidden again during exit. Video drag keeps the same `VideoContent` call site composed across idle and active drag so playback position is preserved; after committed drag release, keep the dragged shared-thumbnail node mounted in the same branch and bounds through the active exit transition while removing only the live video.
...
- Mosaic-enabled Timeline, Album, and Person detail runtime rendering must not show fallback thumbnails for pending, cancelled, failed, or partial-gap work. Completed real chunks render as Mosaic bands; unresolved or invalid ranges render `PlaceholderItem`s and remain resumable. Completed ready fallback renders cropped full-width `RowItem(kind = MOSAIC_FALLBACK)`.
