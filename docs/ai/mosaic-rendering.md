...
- Cache-off Timeline sync is runtime-only from the UI perspective: cold sync, warm changed-bucket sync, and manual sync must not prepare persisted Mosaic artifacts or fall through to runtime compute. Runtime compute is allowed only from explicit render-demand requests such as visible buckets, targeted buckets, and scroll-settled resume.
